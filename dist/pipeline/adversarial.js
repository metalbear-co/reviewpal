...
exports.runAdversarialReview = runAdversarialReview;
...
function getSpecialist(language) {
    switch (language) {
        case 'rust': return RUST_SPECIALIST;
        case 'typescript': return TYPESCRIPT_SPECIALIST;
        case 'python': return PYTHON_SPECIALIST;
        case 'go': return GO_SPECIALIST;
        default: return DEFAULT_SPECIALIST;
    }
}
/**
...
function selectPersonas(files) {
    ...
    return [SECURITY_AUDITOR, REGRESSION_HUNTER, specialist];
}
/**
 ...
 */
async function runAdversarialReview(client, files, triageResult, architectureContext, lessonsContext, config, model, maxCalls) {
    ...
    const allFindings = await Promise.all(personas.map(persona => reviewWithPersona(client, persona, filesToReview, triageResult, architectureContext, lessonsContext, config, model)));
    // Flatten and deduplicate findings
    const findings = allFindings.flat();
    return deduplicateFindings(findings);
}
async function reviewWithPersona(client, persona, files, triageResult, architectureContext, lessonsContext, config, model) {
