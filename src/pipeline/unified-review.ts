...
- crash: The application process will terminate or an unhandled exception will propagate to the user. NOT "a value might be wrong."
  ...
  "adversarialFindings": [
    {
      ...
      "filename": "path/to/file",
      "type": "security|crash|data-loss|performance|regression",
      "line": 42,
  ...
  return raw
    ...
    .map(r => ({
      ...
      critical: Array.isArray(r.critical) ? r.critical.map(c => ({
        type: (c.type || 'security') as DeepReviewResult['critical'][number]['type'],
        line: c.line || 1,
        issue: c.issue || '',
...
function mapAdversarialFindings(
  ...
  return raw
    ...
    .map(f => ({
      ...
      filename: f.filename!,
      type: (f.type || 'security') as AdversarialFinding['type'],
      line: f.line || 1,
      issue: f.issue!,
