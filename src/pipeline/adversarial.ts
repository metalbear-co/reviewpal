...
const SECURITY_AUDITOR: PersonaConfig = {
...
const REGRESSION_HUNTER: PersonaConfig = {
...
const RUST_SPECIALIST: PersonaConfig = {
...
const TYPESCRIPT_SPECIALIST: PersonaConfig = {
...
const PYTHON_SPECIALIST: PersonaConfig = {
...
const GO_SPECIALIST: PersonaConfig = {
...
const DEFAULT_SPECIALIST: PersonaConfig = {
...
function detectPrimaryLanguage(files: DiffFile[]): Language {
  const counts: Record<Language, number> = { rust: 0, typescript: 0, python: 0, go: 0, unknown: 0 };
...
function getSpecialist(language: Language): PersonaConfig {
  switch (language) {
 ...
 */
function selectPersonas(files: DiffFile[]): PersonaConfig[] {
